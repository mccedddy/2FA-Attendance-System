...
if (isset($_POST['date'])) {
  ...
  if ($subject == 'ALL') {
    $sql = "SELECT a.id, a.id_number, a.schedule_id, a.date, s.last_name AS student_last_name, s.first_name AS student_first_name, a.status, p.last_name AS professor_last_name, p.first_name AS professor_first_name, sub.subject_name AS subject_name
          FROM attendance a
          ...
          WHERE a.date = '$date' AND s.section = '$selectedSection' AND a.status = 'Absent'
    ...
    $sql = "SELECT a.id, a.id_number, a.schedule_id, a.date, s.last_name AS student_last_name, s.first_name AS student_first_name, a.status, p.last_name AS professor_last_name, p.first_name AS professor_first_name, sub.subject_name AS subject_name
          ...
          INNER JOIN subjects sub ON sch.subject_code = sub.subject_code
          LEFT JOIN professors p ON sch.professor = p.id_number
          WHERE a.date = '$date' AND s.section = '$selectedSection' AND sub.subject_code = '$subject' AND a.status = 'Absent'
          ORDER BY s.last_name";
  }
